...
describe('OpenExternal URL validation', () => {
	test('allows standard and custom external protocols', () => {
		...
		assert.equal(shouldOpenExternalUrl('https://web.fluxer.app'), true);
	...
	test('opens normalized URLs once inside the dedupe window', async () => {
		...
		await openExternalDeduped('https://web.fluxer.app/path');

		assert.deepEqual(openExternalCalls, ['https://web.fluxer.app/path']);
	});
